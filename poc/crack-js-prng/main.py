#!/usr/bin/python3
# Modified for powerball application
import z3
import struct
import sys
...
for start in range(0,75):
    N = 20
    print(f"Start: {start}")


    sequence = numberlist[start:start+N]
    print(sequence)
    ...
    for i in range(len(sequence)):
        ...
        """
        low_bound = (sequence[i] - 1) / 10000
        high_bound = (sequence[i]) / 10000
        ...
        # We know upper 12 bits of mantissa
        mantissa_low = (u_long_long_64_low & ((1 << 52) - 1)) - 1
        mantissa_high = (u_long_long_64_high & ((1 << 52) - 1)) + 1
        #print(mantissa_low,mantissa_high,mantissa_high-mantissa_low)
        #known_mantissa = mantissa >> (52 - 11)
        #print(mantissa, known_mantissa)
        #print(se_state0)

        # Compare Mantissas
        solver.add(z3.LShR(se_state0,12) >= mantissa_low)
        solver.add(z3.LShR(se_state0,12) <= mantissa_high)
    ...
    if solver.check() == z3.sat:
        ...
        print(next_sequence)
        print(math.floor(next_sequence*10000) + 1)
        print(numberlist[start+N])
